...
### 도메인별 타입
- `src/advanced/domain/product/productTypes.ts` - 상품 관련 타입
- `src/advanced/domain/cart/cartTypes.ts` - 장바구니 관련 타입
- `src/advanced/domain/notification/notificationTypes.ts` - 알림 관련 타입
...
export type FilledCartItem = CartItem & {
  priceDetails: {
    ...
    discountRate: number;      // 할인율 (퍼센트, 0-100)
  };
};
```
