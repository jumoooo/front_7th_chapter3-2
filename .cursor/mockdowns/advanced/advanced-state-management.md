...
모든 상태는 Hook을 통해 관리됩니다. Zustand로 리팩토링 예정입니다.

---

## 🔄 현재 상태 목록

### 1. 상품 관련 상태 (useProduct Hook)
...
- localStorage 동기화는 Store 내부에서 처리
...
Zustand Store
  ...
  │   ├─ productForm
  │   ├─ editingProduct
  │   └─ actions (addProduct, updateProduct, etc.)
  ├─ useCartStore
  │   ├─ cart
  │   ├─ totalItemCount
  ...
  └─ useCouponStore
      ├─ coupons
      ├─ selectedCoupon
      └─ actions (addCoupon, applyCoupon, etc.)

컴포넌트
  └─ Store에서 직접 상태 및 액션 사용
  └─ Props drilling 제거
```
